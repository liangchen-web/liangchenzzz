...
description: |
  N-day 补丁差分与漏洞可利用性验证。从厂商发布的补丁中定位漏洞点、构建 PoC 验证可利用性。
  适用场景：已知 CVE 编号但只有补丁没有 PoC、SRC/红队需要验证未及时更新资产的风险、N-day 可利用性评估、Patch Tuesday 跟进。
  核心方法：拿 before/after 二进制 → 对齐符号 → 二进制 diff → 看新增的安全检查反推 bug class → 构建 PoC 验证漏洞可触发性。
  触发关键词：N-day、Nday、补丁差分、patch diff、patch tuesday、1day、binary diff 漏洞、bindiff、ghidriff、Diaphora、补丁分析、CVE 复现、漏洞还原、补丁反推、N-day PoC。
---
...
6. `ALWAYS`: 遵守渐进式推理纪律（`../../references/progressive-reasoning.md`）——补丁差分按 diff→定位→触发→验证 逐步推进

# N-day 补丁差分与可利用性验证 (Patch Diff Analysis)
...
| 写出完整利用链（堆喷、ROP、提权） | `../sec-pwn-chain/` |
| 把 1-day 武器化部署到目标网络 | `pentest-tools/network-attack-defense/` |
| 从零逆向一个二进制 | `reverse-master` 技能（回退 `../sec-reverse/`） |
...
**上游入口**: `../../SKILL.md`（security-research 总控）、`routing.md`

**上游 skill**:
- `reverse-master` 技能（回退 `../sec-reverse/`） — 在做 diff 之前可能要先理解目标二进制的整体结构
- `binary-diff/` — 如果补丁后版本无符号、补丁前有符号，先用 binary-diff 搬符号过来

**下游 skill**:
- `../sec-pwn-chain/` — 反推出 bug class 后，需要写完整利用（堆喷、ROP、SMEP/SMAP 绕过、提权 payload）
