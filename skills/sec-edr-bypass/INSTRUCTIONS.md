...
name: edr-bypass-re
description: |
  EDR/AV 检测能力评估与防御覆盖验证。在授权红队/对抗演练/自有产品测试场景下，逆向分析防御方实现（hook 表、ETW provider、AMSI）
  以评估检测覆盖范围，并验证针对性的检测规避技术（unhook / 间接 syscall / ETW patch / call stack spoof）。对照 MITRE ATT&CK T1562 防御规避。
  触发关键词：EDR 检测评估、AV 覆盖测试、unhook、direct syscall、indirect syscall、Hell's Gate、Halo's Gate、
  Tartarus Gate、ETW patch、AMSI patch、call stack spoofing、hardware breakpoint Blindside、MITRE T1562、
  ntdll unhook、kernel callback、CrowdStrike 评估、Defender 评估、Sentinel One 评估、Elastic Defend、
  Sysmon 覆盖分析、PPID spoof、Sleep mask、Process Hollowing、Reflective DLL。
---
...
6. `ALWAYS`: 遵守渐进式推理纪律（`../../references/progressive-reasoning.md`）——EDR/AV 研究属于最高敏感度领域，每个验证步骤必须内联范围锚点、验证目的和最小下一步，每回合只升一级

# EDR 检测覆盖评估：从防御方实现分析到覆盖验证

> 仅限授权红队 / 对抗演练 / 自有产品测试，禁止用于未授权目标。

## 适用范围

红队 / 对抗模拟在已获授权的目标主机评估 EDR 检测能力并验证规避技术时使用本 skill。

1. **红队 / Purple team / 对抗演练** — 客户希望评估 SOC 与 EDR 的真实检测能力
2. **自研 implant / C2 框架研发** — 开发针对自家产品测试的载荷，需要验证目标 EDR 的检测边界
3. **EDR 产品评估** — 在合规边界已确认的前提下，客观评测某款 EDR 的检测覆盖
4. **CTF / 攻防演练的 Windows 端突破** — 比赛中需要在加固主机上验证执行可行性

**不适用场景**：

- 杀毒厂商对自家产品做完整 RE 给客户出商业评估报告（找厂商正式合作）
- 未授权目标的检测规避（违法）
- 普通恶意代码的规避技术（本 skill 关注红队 OPSEC 下的检测能力评估，不教恶意代码写法）
...
- `reverse-master` 技能（回退 `../sec-reverse/`） — 需要先理解 EDR DLL / 驱动的实现
...
- `../../references/experience-index.md` — 每次实战后沉淀经验
