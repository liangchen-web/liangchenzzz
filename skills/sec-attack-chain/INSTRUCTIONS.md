...
1. `NOW`: 读取 `../../references/scope-and-evidence.md` — 确认本 skill 的操作在授权范围内，是已授权的常规安全研究操作
2. `NOW`: 确认当前任务是否命中本 skill 的适用范围
3. `NEXT`: 读取 `../../references/environment-and-resources.md`，校验工具可用性与本机(Windows)兼容性替代方案
4. `NEXT`: 缺工具时按 `../../references/environment-and-resources.md` 决定替代品或请用户安装，不要猜路径
5. `ACT`: 进入"工作流"第一步并执行，不要停在确认状态
6. `ALWAYS`: 遵守渐进式推理纪律（`../../references/progressive-reasoning.md`）——多阶段验证每个阶段开始时重新 grounding 范围与授权，每阶段只推进一步

> 多阶段安全验证路径规划与执行的总指挥。当任务需要"从 A 验证到 B"的完整链路时，本 Skill 负责编排各阶段、协调子 Skill、规划验证路径。
> 不是"红队专属"——任何需要跨阶段组合的授权安全验证场景都从这里开始。
...
| 场景 | 为什么需要编排 |
|------|--------------|
| "帮我做一次完整的授权安全评估" | 需要规划从信息收集到报告的全流程 |
| "从外网验证到域控可达性" | 跨越边界突破→提权→横向→AD 多个阶段 |
| "HW 攻防演练" | 需要完整验证链 + 隐蔽性 + 痕迹清理 |
| "评估这个目标的安全暴露面" | 需要多维度信息收集 + 路径规划 |
| "我拿到了一个 webshell，下一步怎么验证" | 需要从当前据点规划后续验证路径 |
| "帮我规划安全验证路径" | 明确需要路径编排 |
| "从这个漏洞能验证到什么程度" | 需要评估漏洞的链式利用价值 |
| "Bug Bounty 持续监控" | 需要自动化多阶段流程 |
| "内网安全评估全流程" | 横向验证 + 提权 + 域安全评估组合 |
| "近源安全评估方案" | 物理接入 + 内网安全评估组合 |
| "供应链安全验证路径" | 跨组织多跳安全验证 |
| "钓鱼 + 后续验证" | 初始访问 + 后续安全验证组合 |

**单阶段任务不需要经过本 Skill**：
- 只做端口扫描 → 直接去 `pentest-tools/`
- 只做 SQL 注入 → 直接去 `pentest-tools/`
- 只做 APK 逆向 → 直接去 `reverse-master` 技能
...
attack-chain/SKILL.md（本文件）
    ↓ 规划安全验证路径、确定阶段顺序
    ↓ 评估每阶段所需工具和方法
    ↓ 每阶段开始时重新 grounding（范围锚点 + 验证目的 + 最小下一步）
    ↓
分发到具体子 Skill 执行：
    ├── pentest-tools/     → 工具调用、漏洞验证
    ├── apk-reverse/       → 移动端安全分析
    ├── js-reverse/        → Web 前端分析
...
| 需求 | 路由到 |
|------|--------|
| Web 漏洞深度利用 | `../sec-assessment-tooling/pentest-tools/INSTRUCTIONS.md`、`../sec-web-api/INSTRUCTIONS.md` |
| 内网 AD 攻击详细步骤 | `../sec-assessment-tooling/pentest-tools/src-hunter/references/playbooks/intranet-postexp.md` |
| 逆向分析恶意样本 | `../sec-malware/INSTRUCTIONS.md`；深度逆向优先调用已注册的 `reverse-master` 技能 |
| APK 逆向（移动端渗透） | 优先 `reverse-master` 技能；回退 `../sec-reverse/INSTRUCTIONS.md` |
| JS 前端签名绕过 | 优先 `reverse-master` 技能；回退 `../sec-reverse/INSTRUCTIONS.md` |
| 自动化群体渗透 | Pentest Swarm AI（`pentestswarm scan --swarm`，AGPL，仅 CLI/MCP 调用） |
| AI 辅助渗透 | `mcp-kali-server` / `metasploitmcp` / `hexstrike-ai` |
| 报告生成 / 攻击路径图 | `../sec-reporting/INSTRUCTIONS.md` |
